    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-form :form="form">
        ...
        <div v-show="currentStep === 0">
          <a-form-item label="所属学科" :labelCol="labelCol" :wrapperCol="wrapperCol">
            <a-select v-decorator="['categorie', {rules: [{required: true}]}]" placeholder="请选择学科" style="width: 100%">
              <a-select-option v-for="categorie in categories" :value="categorie.id" :key="categorie.id">
                {{ categorie.name }}
              </a-select-option>
            </a-select>
          </a-form-item>	
          ...
          >
            <a-input-number
              :min="1"
              :max="200"
              v-decorator="['elapse', {initialValue: '20',rules: [{required: true}]}]"
            />
            分钟
          </a-form-item>
          <a-form-item
        ...
        <div v-show="currentStep === 2">
          ...
          >
            ...
            >
              <a-select-option v-for="radio in radiosFilter" :value="radio.name" :key="radio.id">
                {{ radio.name }}
              </a-select-option>
            </a-select>
          </a-form-item>
          ...
          >
            ...
            >
              <a-select-option v-for="judge in judgesFilter" :value="judge.name" :key="judge.id">
                {{ judge.name }}
              </a-select-option>
            </a-select>
          </a-form-item>
...
export default {
  ...
  methods: {
    ...
    handleNext (step) {
      ...
      if (currentStep == 1) {
        ...
        validateFields(stepForms[0], (errors, values) => {
          if (!errors) {
            console.log('获取学科:', values.categorie)
      ...
      validateFields((errors, values) => { // values就是表单中校验的值，后面提交到后端接口时主要就是用这个values
        ...
        values.radios = this.radiosFilter
        values.judges = this.judgesFilter
        console.log('提交数据到后端')
    ...
    handleRadioChange (values) {
      console.log(values)
      // 更新单选题的信息
      for (let i = 0; i < this.radiosFilter.length; i++) { // 遍历所有的题目的选项
        // 取出一个选项的id
        const name = this.radiosFilter[i].name
        // 当前问题是否被问题创建者选中
        let checked = false
        for (let j = 0; j < values.length; j++) { // 拿着
          ...
          if (name === value) {
            // 说明这个问题被考试创建者选中
            checked = true
            this.radiosFilter[i].checked = true
            break // 跳出内部的for循环
          }
        }
        // 这个选项遍历到最后，发现还不是答案(不在答案数组中)，那么就把这个选项的answer属性设置为false
        if (checked === false) {
          this.radiosFilter[i].checked = false
        }
      }
    },
    ...
    handleCheckChange (values) {
      console.log(values)
      // 更新单选题的信息
      for (let i = 0; i < this.checksFilter.length; i++) { // 遍历所有的题目的选项
        // 取出一个选项的id
        const name = this.checksFilter[i].name
        // 当前问题是否被问题创建者选中
        let checked = false
        for (let j = 0; j < values.length; j++) { // 拿着
          const value = values[j]
          if (name === value) {
            // 说明这个问题被考试创建者选中
            checked = true
            this.checksFilter[i].checked = true
            break // 跳出内部的for循环
          }
        }
        // 这个选项遍历到最后，发现还不是答案(不在答案数组中)，那么就把这个选项的answer属性设置为false
        if (checked === false) {
          this.checksFilter[i].checked = false
        }
      }
    },
    ...
    handleJudgeChange (values) {
      console.log(values)
      // 更新单选题的信息
      for (let i = 0; i < this.judgesFilter.length; i++) { // 遍历所有的题目的选项
        // 取出一个选项的id
        const name = this.judgesFilter[i].name
        // 当前问题是否被问题创建者选中
        let checked = false
        for (let j = 0; j < values.length; j++) { // 拿着
          const value = values[j]
          if (name === value) {
            // 说明这个问题被考试创建者选中
            checked = true
            this.judgesFilter[i].checked = true
            break // 跳出内部的for循环
          }
        }
        // 这个选项遍历到最后，发现还不是答案(不在答案数组中)，那么就把这个选项的answer属性设置为false
