<template>
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="float: right;">
        ...
        <a-popconfirm
          ...
          ok-text="确认交卷"
          cancel-text="继续答题"
          style="margin-right: 60px; font-size: 20px; color: red; border: 2px solid red;"
          @confirm="finishExam()"
          @cancel="cancel()"
        >
          <a href="#">交卷</a>
    ...
    <a-progress :percent="this.percent" status="active" />
    <a-layout>
      <a-layout-sider width="190" :style="{ background: '#444', overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }">
        <a-menu mode="inline" :defaultSelectedKeys="['1']" :defaultOpenKeys="['question_radio', 'question_check', 'question_judge']" :style="{ height: '100%', borderRight: 0 }">
          <a-sub-menu key="question_radio" v-if="examDetail.radioIds[0] != ''">
            <span slot="title" v-if="examDetail.exam">
              <a-icon type="check-circle" theme="twoTone" />
              单选题(每题{{ examDetail.exam.examScoreRadio }}分)
            </span>
          ...
          -->
          <a-sub-menu key="question_judge" v-if="examDetail.judgeIds[0] != ''">
            <span slot="title" v-if="examDetail.exam">
              <a-icon type="like" theme="twoTone" />
              判断题(每题{{ examDetail.exam.examScoreJudge }}分)
            </span>
      ...
      <a-layout :style="{ marginLeft: '200px' }">
        ...
        <a-layout-footer :style="{ textAlign: 'center' }">©2026 Crated by Lu Xu Hao</a-layout-footer>
      </a-layout>
    </a-layout>
  </a-layout>
</template>
...
export default {
  ...
  methods: {
    ...
    onRadioChange(e) {
      ...
      // 计算进度条的总题目
      let tatalRadioIdsNum = 0;
      let tatalJudgeIdsNum = 0;
      if (this.examDetail.radioIds[0] === '') {
        tatalRadioIdsNum = 0;
      } else {
        tatalRadioIdsNum = this.examDetail.radioIds.length;
      }
      if (this.examDetail.judgeIds[0] === '') {
        tatalJudgeIdsNum = 0;
      } else {
        tatalJudgeIdsNum = this.examDetail.judgeIds.length;
      }
      const tatal = tatalRadioIdsNum + tatalJudgeIdsNum;
      // 计算进度条当前完成的题目
      const num = this.answersMap.size;
      this.percent = Math.round((num / tatal) * 100);
    },
    /**
     * 多选题触发的变化事件
     * @param checkedValues
