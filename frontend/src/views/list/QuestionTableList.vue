    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: 100,
          formatter: (value, row) => {
            return '<div class="question-grade">' + value + '</div>'
          },
          events: {
            'click .question-grade': function (e, value, row, index) {
  ...
  methods: {
    ...
    dblClickCell (field, value, row, $element) {
      if (field === 'gradeVolumeName') { // 更新教材范围
        const childrenSelect = $element.children('.question-grade').children('select') // 获取输入框的值
        ...
        questionUpdate(row).then(res => {
          ...
          console.log(res)
          if (res.code === 0) {
            $element.children('.question-grade').text(row.gradeVolumeName)
