...
public class UserServiceImpl implements UserService {
    ...
    public UserInfoVo getInfo(String userId) {
        ...
        if(gradeLevel==null){
            userInfoVo.setUserGradeLevelName("");
        }else{
        ...
        // 设置班级信息
        Integer userClassId = user.getUserClassId();
        UserClass userClass = userClassRepository.findById(userClassId).orElse(null);
        if(userClass==null){
            userInfoVo.setUserClassName("");
        }else{
            userInfoVo.setUserClassId(userClass.getClassId());
            userInfoVo.setUserClassName(userClass.getClassName());
        }
        return userInfoVo;
    }
}
