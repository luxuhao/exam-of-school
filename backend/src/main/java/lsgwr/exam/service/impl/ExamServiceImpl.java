...
public class ExamServiceImpl implements ExamService {
    ...
    private final GradeVolumeRepository gradeVolumeRepository;
    ...
    private QuestionVo getQuestionVo(Question question) {
        ...
        // 设置题目所属上下册，比如五年级上册、五年级下册
        questionVo.setGradeVolumeName(
                Objects.requireNonNull(
                        gradeVolumeRepository.findById(
                                question.getGradeVolumeId()
                        ).orElse(null)
                ).getGradeVolumeName()
        );
    ...
    private List<ExamVo> getExamVos(List<Exam> examList) {
        ...
        for (Exam exam : examList) {
            ...
            examVo.setState(
                    Objects.requireNonNull(
                            examTypeRepository.findById(
                                    exam.getExamTypeId()
                            ).orElse(null)
                    ).getExamTypeName()
            );
            // 设置试卷的学科名字
            examVo.setExamCategoryId(exam.getExamQuestionCategoryId());
            examVo.setExamCategoryName(
                    Objects.requireNonNull(
                    questionCategoryRepository.findById(
                            exam.getExamQuestionCategoryId()
                    ).orElse(null)
            ).getQuestionCategoryName()
            );

//            if(ExamTypeEnum.State0.getType().equals(examVo.getIsTurnOn())){
//                examVo.setState(ExamTypeEnum.State0.getName());
//            }else {
//                examVo.setState(ExamTypeEnum.State1.getName());
//            }

            // 设置题目所属上下册，比如五年级上册、五年级下册
            examVo.setGradeVolumeName(
                    Objects.requireNonNull(
                            gradeVolumeRepository.findById(
                                    examVo.getGradeVolumeId()
                            ).orElse(null)
                    ).getGradeVolumeName()
            );
...
//            examVo.setExamQuestionSelectVoCheckList(checkQuestionVoList);
    ...
    public ExamQuestionTypeVo getExamQuestionType() {
        ...
        for (Question question : radioQuestionList) {
            ExamQuestionSelectVo radioQuestionVo = new ExamQuestionSelectVo();
            BeanUtils.copyProperties(question, radioQuestionVo);
            radioQuestionVo.setGradeVolumeId(question.getGradeVolumeId());
            radioQuestionVo.setQuestionCategory(question.getQuestionCategoryId());
            radioQuestionVoList.add(radioQuestionVo);
        }
...
//        examQuestionTypeVo.setExamQuestionSelectVoCheckList(checkQuestionVoList);
        ...
        for (Question question : judgeQuestionList) {
            ExamQuestionSelectVo judgeQuestionVo = new ExamQuestionSelectVo();
            BeanUtils.copyProperties(question, judgeQuestionVo);
            judgeQuestionVo.setGradeVolumeId(question.getGradeVolumeId());
            judgeQuestionVo.setQuestionCategory(question.getQuestionCategoryId());
            judgeQuestionVoList.add(judgeQuestionVo);
        }
...
//        }
        ...
        exam.setExamQuestionIds(radioIdsStr + "-" + judgeIdsStr);
        // 设置各个题目的id
        ...
        int examScore = radioCnt * exam.getExamScoreRadio() + judgeCnt * exam.getExamScoreJudge();
...
//        checkIdsStr = replaceLastSeparator(checkIdsStr);
    ...
    @Override
    public List<ExamCardVo> getExamCardList(Integer userGradeLevelId, String userId) {
        //传入参数为当前用户的年段 ID
        List<Exam> examList = examRepository.findAll();
        List<ExamCardVo> examCardVoList = new ArrayList<>();
        for (Exam exam : examList) {
            ExamCardVo examCardVo = new ExamCardVo();
            //获取匹配当前用户的考试： 如果是学生匹配 年段和已启用； 如果是老师则全展示
            BeanUtils.copyProperties(exam, examCardVo);
            GradeVolume gradeVolume = gradeVolumeRepository.findById(exam.getGradeVolumeId()).orElse(null);
            if(userGradeLevelId.equals(gradeVolume.getGradeLevelId()) && (exam.getExamTypeId().intValue() == 2)){
                //查询当前用户这份试卷是否有提交成绩
                List<ExamRecordVo> examRecordVoList = getExamRecordList(userId);
                for (ExamRecordVo examRecordVo : examRecordVoList) {
                    if(exam.getExamId().equals(examRecordVo.getExam().getExamId())){
                        // 说明已经有考试记录，做过了,1代表已经做过了，0代表没做过
                        examCardVo.setSubmitted(1);
                    }
                }
